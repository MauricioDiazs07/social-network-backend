...
def ModeloIne(filename):
    with open('model_read.pkl', 'rb') as f:
        reader2 = pickle.load(f)
    
    INE = Image.open(filename)
    width, height = INE.width, INE.height

    cabeceraRecorte = INE.crop((width*0.0, height*0.0, width*1.0, height*0.26))
    cabeceraRecorte.save("INE-cabecera.png")

    credencialValida = False
    result = reader2.readtext("INE-cabecera.png", detail = 0)
    ...
    if credencialValida:
        ...
        otrosRecorte.save("INE-otros.png")
        result = reader2.readtext("INE-otros.png", detail = 0)
    ...
    if credencialValida and emision < 2020:
        INE.crop((width*0.075, height*0.36, width*0.31, height*0.85)).save("INE-foto.png")
        
        INE.crop((width*0.28, height*0.26, width*0.7, height*0.46)).save("INE-nombre.png")
        nombre = reader2.readtext("INE-nombre.png", detail = 0)[1:]
        nombre = " ".join(nombre).upper()
        
        INE.crop((width*0.30, height*0.44, width*0.825, height*0.645)).save("INE-domicilio.png")
        domicilio = reader2.readtext("INE-domicilio.png", detail = 0)[1:]
        domicilio = " ".join(domicilio).upper()

        INE.crop((width*0.91, height*0.335, width*0.97, height*0.445)).convert('L').save("INE-genero.png") 
        genero = reader2.readtext(f"INE-genero.png", detail = 0)
        ...
        INE.crop((width*0.76, height*0.28, width*0.98, height*0.365)).save("INE-fechaDeNacimiento.png")
        fechaDeNacimiento = reader2.readtext("INE-fechaDeNacimiento.png", detail = 0)
        ...
        print (f"fecha de nacimiento: {fechaDeNacimiento}")

        
        INE.crop((width*0.29, height*0.695, width*0.68, height*0.78)).save("INE-curp.png")
        curp = reader2.readtext("INE-curp.png", detail = 0)
        if (len(curp) >= 1):
        ...
        INE.crop((width*0.49, height*0.635, width*0.82, height*0.725)).save("INE-claveDeElector.png")
        INE.crop((width*0.855, height*0.685, width*0.98, height*0.805)).save("INE-añoDeRegistro.png")
        INE.crop((width*0.39, height*0.76, width*0.50, height*0.84)).save("INE-estado.png")
        INE.crop((width*0.615, height*0.76, width*0.70, height*0.84)).save("INE-municipio.png")
        INE.crop((width*0.78, height*0.760, width*0.87, height*0.84)).save("INE-seccion.png")
        INE.crop((width*0.43, height*0.835, width*0.52, height*0.93)).save("INE-localidad.png")
        INE.crop((width*0.61, height*0.83, width*0.705, height*0.93)).save("INE-emision.png")
        INE.crop((width*0.78, height*0.825, width*0.88, height*0.93)).save("INE-vigencia.png")

    if credencialValida and emision >= 2020:
        INE.crop((width*0.075, height*0.36, width*0.31, height*0.85)).save("INE-foto.png")
        
        INE.crop((width*0.28, height*0.26, width*0.7, height*0.52)).save("INE-nombre.png")
        nombre = reader2.readtext("INE-nombre.png", detail = 0)
        if (len(nombre) >= 1):
            nombre = nombre[1:]
        ...
        INE.crop((width*0.30, height*0.49, width*0.825, height*0.705)).save("INE-domicilio.png")
        domicilio = reader2.readtext("INE-domicilio.png", detail = 0)
        if (len(domicilio) >= 1):
            domicilio = domicilio[1:]
        domicilio = " ".join(domicilio)

        INE.crop((width*0.83, height*0.25, width*0.98, height*0.38)).save("INE-genero.png")
        genero = reader2.readtext(f"INE-genero.png", detail = 0)
        for elemento in genero:
            if "H" in elemento:
                genero = "H"
        ...
        INE.crop((width*0.30, height*0.74, width*0.67, height*0.855)).save("INE-curp.png")
        curp = reader2.readtext("INE-curp.png", detail = 0)
        if (len(curp) >= 1):
            for i, element in enumerate(curp): #primer caso checa por el siguiente elemento despues de curp
                if element.upper() == "CURP" and len(curp) >= i+2:
        ...
        INE.crop((width*0.30, height*0.84, width*0.57, height*0.96)).save("INE-fechaDeNacimiento.png")
        fechaDeNacimiento = reader2.readtext("INE-fechaDeNacimiento.png", detail = 0)
        if len(fechaDeNacimiento) >= 1 and len(fechaDeNacimiento[-1]) >= 8:
            fechaDeNacimiento = fechaDeNacimiento[-1] 
            diaDeNacimiento = fechaDeNacimiento[0:2]
    ...
    INE.crop((width*0.30, height*0.76, width*0.50, height*0.84)).save(f"INE-estado.png")
    estado = reader2.readtext(f"INE-estado.png", detail = 0)
