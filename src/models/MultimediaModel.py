...
GET_ALL_MULTIMEDIA_PROFILE = """ SELECT "PROFILE_ID", "SHARE_ID", "SHARE_TYPE", "ARCHIVE_URL", "ARCHIVE_TYPE" FROM "T_MULTIMEDIA" WHERE "PROFILE_ID" = %s """
DELETE_MULTIMEDIA = """ DELETE FROM "T_MULTIMEDIA" WHERE "SHARE_ID" = %s AND "SHARE_TYPE" = %s """
DELETE_ALL_MULTIMEDIA = """ DELETE FROM "T_MULTIMEDIA" WHERE "PROFILE_ID" = %s """
...
class MultimediaModel():
    ...
    @classmethod
    def get_all_multimedia_from_profile(self, profile_id):
        try:
            conn = get_connection()
            multimedia_list = []
            with conn.cursor() as cur:
                cur.execute(GET_ALL_MULTIMEDIA_PROFILE,(profile_id,))
                resultset = cur.fetchall()
                for row in resultset:
                    multimedia = Multimedia(row[0],row[1],row[2],row[3],row[4])
                    multimedia_list.append(multimedia.to_JSON())
    ...
    @classmethod
    def delete_all_multimedia(self, profile_id):
        try:
            conn = get_connection()
            with conn.cursor() as cur:
                cur.execute(DELETE_ALL_MULTIMEDIA,(profile_id,))
                affected_row = cur.rowcount
                conn.commit()
            conn.close()
            return affected_row
